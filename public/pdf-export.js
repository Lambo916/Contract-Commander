...
(function (ns) {
  ...
  ns.drawFooter = function drawFooter(doc, pageNumber, pageCount) {
    ...
    const footer = `Generated by ${window.currentToolkitName || 'YourBizGuru'}   Page ${pageNumber} of ${pageCount}`;
...
(() => {
  ...
  const TYPOGRAPHY = {
    ...
    // Colors (RGB arrays for jsPDF) - YBG Branding
    colorBody: [33, 33, 33],        // #212121 dark gray text
    colorHeading: [17, 17, 17],     // #111 almost black
    colorMeta: [102, 102, 102],     // #666 muted gray
    colorBlue: [77, 182, 231],      // #4DB6E7 YBG primary blue
    colorYellow: [255, 235, 59],    // #FFEB3B YBG accent yellow/gold
    colorGold: [255, 235, 59]       // Same as yellow for consistency
  };
  ...
  function drawHeader(doc, pageNum, iconDataUrl) {
    ...
    // Add subtle gold divider line below header (YBG yellow)
  ...
  function parseContent(text) {
    ...
    while (i < lines.length) {
      ...
      // Check for premium BizPlan sections
      if (trimmed === '##EXECUTIVE_SNAPSHOT##') {
        const items = [];
        i++;
        while (i < lines.length && lines[i].trim() !== '##END_EXECUTIVE_SNAPSHOT##') {
          const itemLine = lines[i].trim();
          if (itemLine && itemLine.includes(':')) {
            const [label, ...valueParts] = itemLine.split(':');
            items.push({ label: label.trim(), value: valueParts.join(':').trim() });
          }
          i++;
        }
        blocks.push({ type: 'executive_snapshot', items });
        i++; // Skip END marker
        continue;
      }
      
      if (trimmed === '##KPI_TABLE##') {
        const rows = [];
        let headers = [];
        i++;
        while (i < lines.length && lines[i].trim() !== '##END_KPI_TABLE##') {
      ...
      if (trimmed === '##AI_INSIGHTS##') {
        const insights = [];
        i++;
        while (i < lines.length && lines[i].trim() !== '##END_AI_INSIGHTS##') {
          const insightLine = lines[i].trim();
          if (insightLine && insightLine.startsWith('•')) {
            insights.push(insightLine.substring(1).trim());
          }
          i++;
        }
        blocks.push({ type: 'ai_insights', insights });
        i++; // Skip END marker
        continue;
      }
  ...
  const TABLE_STYLES = {
    // YBG Branding colors
    headerBg: [77, 182, 231],        // Blue #4DB6E7 for KPI tables
    headerBgAlt: [255, 235, 59],     // Yellow #FFEB3B for alternate tables
    headerText: [255, 255, 255],     // White text on blue header
    headerTextAlt: [33, 33, 33],     // Dark text on yellow header
  ...
  class PDFWriter {
    ...
    writeExecutiveSnapshot(items) {
      ...
      for (const item of items) {
        const labelWidth = this.doc.getTextWidth(item.label.toUpperCase() + ': ');
        const maxValueWidth = CONTENT.width - labelWidth - 12;
        const valueLines = wrapText(this.doc, item.value, maxValueWidth, 10);
        const itemHeight = Math.max(lineHeight, valueLines.length * lineHeight);
        itemHeights.push({ lines: valueLines, height: itemHeight, labelWidth });
        totalContentHeight += itemHeight;
      }
      
      const boxHeight = totalContentHeight + (boxPadding * 2);
      ...
      // Draw blue accent bar on left (like in-app)
      this.doc.setFillColor(...TYPOGRAPHY.colorBlue);
      this.doc.rect(CONTENT.left, boxStartY, 4, boxHeight, 'F');
      
      // Draw subtle gradient background box (light blue)
      this.doc.setFillColor(240, 248, 255); // Very light blue
      this.doc.rect(CONTENT.left + 4, boxStartY, CONTENT.width - 4, boxHeight, 'F');
      
      // Draw border
      this.doc.setDrawColor(200, 220, 240);
      ...
      this.doc.text("EXECUTIVE SNAPSHOT", CONTENT.left + 6, this.yPosition + 5);
      this.yPosition += titleHeight;
      
      // Draw items with proper multi-line wrapping
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        const { lines, height, labelWidth } = itemHeights[i];
        
        // Label in bold
        this.doc.setFont(TYPOGRAPHY.fontFamily, "bold");
        this.doc.setFontSize(10);
        this.doc.setTextColor(80, 80, 80);
        this.doc.text(item.label.toUpperCase() + ':', CONTENT.left + 6, this.yPosition + 4);
        
        // Value lines in regular
        this.doc.setFont(TYPOGRAPHY.fontFamily, "normal");
        this.doc.setTextColor(...TYPOGRAPHY.colorBody);
        
        let lineY = this.yPosition + 4;
        for (const line of lines) {
          this.doc.text(line, CONTENT.left + 6 + labelWidth, lineY);
          lineY += lineHeight;
        }
      ...
      this.yPosition += boxPadding + 3; // Extra space after box
    ...
    writeKpiTable(data) {
      ...
      // Calculate column widths (equal distribution for KPI table)
      ...
      this.doc.setFontSize(12);
      this.doc.setTextColor(...TYPOGRAPHY.colorHeading);
      this.doc.text("Key Performance Indicators (KPIs)", CONTENT.left, this.yPosition);
      this.yPosition += 5;
      
      // Gold underline
      this.doc.setDrawColor(...TYPOGRAPHY.colorYellow);
      this.doc.setLineWidth(0.6);
      this.doc.line(CONTENT.left, this.yPosition, CONTENT.left + 80, this.yPosition);
      this.yPosition += 4;
      
      // Check space for header + first row
      if (this.needsNewPage(TABLE_STYLES.headerHeight + TABLE_STYLES.rowHeight + 5)) {
        this.addNewPage();
      }
      ...
      for (let i = 0; i < headers.length; i++) {
        // Blue background
        this.doc.setFillColor(...TYPOGRAPHY.colorBlue);
        this.doc.rect(xPos, this.yPosition, columnWidths[i], TABLE_STYLES.headerHeight, 'F');
        
        // White text
        this.doc.setFont(TYPOGRAPHY.fontFamily, "bold");
        this.doc.setFontSize(TABLE_STYLES.headerFontSize);
        this.doc.setTextColor(255, 255, 255);
        
        // Center text in cell
        const textWidth = this.doc.getTextWidth(headers[i]);
        const textX = xPos + (columnWidths[i] - textWidth) / 2;
        this.doc.text(headers[i], textX, this.yPosition + 6);
        
        // Border
        this.doc.setDrawColor(...TABLE_STYLES.borderColor);
        this.doc.setLineWidth(TABLE_STYLES.borderWidth);
        this.doc.rect(xPos, this.yPosition, columnWidths[i], TABLE_STYLES.headerHeight);
        
        xPos += columnWidths[i];
      }
      this.yPosition += TABLE_STYLES.headerHeight;
      
      // Draw data rows with alternating shading
      for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
        ...
        for (let colIdx = 0; colIdx < row.length; colIdx++) {
          ...
          if (isZebra) {
            this.doc.setFillColor(...TABLE_STYLES.zebraColor);
            this.doc.rect(xPos, this.yPosition, columnWidths[colIdx], rowHeight, 'F');
          }
          
          // Cell border
          this.doc.setDrawColor(...TABLE_STYLES.borderColor);
          this.doc.setLineWidth(TABLE_STYLES.borderWidth);
          this.doc.rect(xPos, this.yPosition, columnWidths[colIdx], rowHeight);
          
          // Cell text
          this.doc.setFont(TYPOGRAPHY.fontFamily, "normal");
          this.doc.setFontSize(TABLE_STYLES.fontSize);
          this.doc.setTextColor(...TYPOGRAPHY.colorBody);
          
          // Wrap text if needed
          const maxTextWidth = columnWidths[colIdx] - (TABLE_STYLES.cellPadding * 2);
          const wrappedLines = wrapText(this.doc, cellText, maxTextWidth, TABLE_STYLES.fontSize);
          
          // Draw wrapped lines
          let textY = this.yPosition + TABLE_STYLES.cellPadding + 3;
          for (const line of wrappedLines) {
            this.doc.text(line, xPos + TABLE_STYLES.cellPadding, textY);
            textY += 4;
          }
    ...
    writeAiInsights(insights) {
      ...
      const insightHeights = [];
      for (const insight of insights) {
        const maxTextWidth = CONTENT.width - 15;
        const wrappedLines = wrapText(this.doc, insight, maxTextWidth, 10);
        const itemHeight = wrappedLines.length * lineHeight + 1; // +1 for spacing
        insightHeights.push({ lines: wrappedLines, height: itemHeight });
        totalContentHeight += itemHeight;
      }
      ...
      // Draw yellow gradient background box
      this.doc.setFillColor(255, 252, 230); // Very light yellow
      this.doc.rect(CONTENT.left, boxStartY, CONTENT.width, boxHeight, 'F');
      
      // Draw gold left border
      this.doc.setFillColor(...TYPOGRAPHY.colorYellow);
      this.doc.rect(CONTENT.left, boxStartY, 3, boxHeight, 'F');
      
      // Draw border
      this.doc.setDrawColor(255, 220, 100);
      this.doc.setLineWidth(0.5);
      this.doc.rect(CONTENT.left, boxStartY, CONTENT.width, boxHeight);
      
      this.yPosition += boxPadding;
      
      // Title (no emoji - use plain text for PDF compatibility)
      this.doc.setFont(TYPOGRAPHY.fontFamily, "bold");
      this.doc.setFontSize(12);
      this.doc.setTextColor(...TYPOGRAPHY.colorHeading);
      this.doc.text("AI INSIGHTS", CONTENT.left + 5, this.yPosition + 5);
      this.yPosition += titleHeight;
      
      // Draw insights as bullet list with proper multi-line wrapping
      this.doc.setFont(TYPOGRAPHY.fontFamily, "normal");
      this.doc.setFontSize(10);
      this.doc.setTextColor(...TYPOGRAPHY.colorBody);
      
      for (const { lines, height } of insightHeights) {
        // Draw bullet
      ...
      this.doc.text("Generated insights provided by YourBizGuru AI.", CONTENT.left + 5, this.yPosition + 3);
    ...
    writeBlock(block) {
      // Handle premium BizPlan sections
      if (block.type === 'executive_snapshot') {
        this.writeExecutiveSnapshot(block.items);
        return;
      }
      
      if (block.type === 'kpi_table') {
        this.writeKpiTable(block.data);
        return;
      }
  ...
  function htmlToMarkdown(html) {
    ...
    function processNode(node) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        // Special handling for BizPlan premium sections
        if (node.classList && node.classList.contains('executive-snapshot')) {
          markdown += '\n##EXECUTIVE_SNAPSHOT##\n';
          const items = node.querySelectorAll('.snapshot-item');
          for (const item of items) {
            const label = item.querySelector('.snapshot-label')?.textContent?.trim() || '';
            const value = item.querySelector('.snapshot-value')?.textContent?.trim() || '';
            if (label && value) {
              markdown += `${label}: ${value}\n`;
            }
          }
          markdown += '##END_EXECUTIVE_SNAPSHOT##\n\n';
          return;
        }
        
        if (node.classList && node.classList.contains('kpi-table-container')) {
          markdown += '\n##KPI_TABLE##\n';
          ...
          markdown += '##END_KPI_TABLE##\n\n';
          return;
        }
        
        if (node.classList && node.classList.contains('ai-insights-box')) {
          markdown += '\n##AI_INSIGHTS##\n';
